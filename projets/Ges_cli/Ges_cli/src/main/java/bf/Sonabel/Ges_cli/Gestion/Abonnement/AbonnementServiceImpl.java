...
public class AbonnementServiceImpl implements AbonnementService {
    ...
    public AbonnementDTO createAbonnement(AbonnementDTO request) {
        // Vérifier l'existence du branchement
        ...
        if (abonnementRepository.existsByBranchementAndDateFinIsNull(branchement)) {
        ...
        if (abonnementRepository.existsByNumPolAndDateFinIsNull(request.getNumPol())) {
            throw new IllegalStateException("Un abonnement actif existe déjà pour ce numéro de police !");
        }

        // Définir la date de début et le statut
    ...
    public AbonnementDTO getAbonnementActifPourResiliation(Integer codeExpl, Integer lot, Integer parcelle,
        ...
        Abonnement abonnement = branchement.getAbonnement();
        if (abonnement == null || abonnement.getDateFin() != null || abonnement.getStatut() != StatutAbonnement.ACTIF) {
            throw new IllegalStateException("Aucun abonnement actif valide trouvé pour ce branchement !");
        }
