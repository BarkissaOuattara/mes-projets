...
public class TypeClientController {
    ...
    @GetMapping
    public ResponseEntity<List<TypeClientDTO>> getAll() {
    List<TypeClientDTO> dtos = typeClientService.getAll().stream()
   ...
   @PutMapping
    ...
    @GetMapping("/search")
    public ResponseEntity<TypeClient> getByLibelle(@RequestParam String libelle) {
    try {
        TypeClient typeClient = typeClientService.getByLibelle(libelle);
        return ResponseEntity.ok(typeClient);
